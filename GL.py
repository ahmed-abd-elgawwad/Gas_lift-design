...
def all_in_one(total_depth,wellhead_pressure, Pcs ,Pko,Glf,Gs,Q,BHSP,J,Tre,Ts,R):
    ...
    Ct_data= pd.read_csv("data/Ct_Data.csv")
    Tg= ((Tre-Ts) / total_depth)
    df["Temp"] = np.round(Ts + Tg * df["Depth"],2)
    df["Pdt"] = np.round((1-R)*df["P1"] + R*df["P2"],2)
...
# ---------------------------------------- end canvas drawing ----------------------------    
FORM_CLASS,_ =loadUiType(path.join(path.dirname(__file__),"Ui Files/final.ui"))
class MainApp(QMainWindow,FORM_CLASS):
    ...
    def save_file(self):
        ...
        if (file_name != "") & ( self.dataframe.empty == False):
            ...
            QMessageBox.information(self,"success","The file is made.")
            # os.system(f'start "excel" "{file_path}"')
        else:
            QMessageBox.warning(self,"warning","please,enter the file name and the required data")
    ...
    def grapgh(self,df,Pwf,Pwh1,Pwh2,Pcs,Pko,total_Depth,reservoir_line):
        ...
        self.mat.axis.annotate(f'Injection point \n {df["Depth"].iloc[-1]} ft', xy=(df["P2"].iloc[-1], df["Depth"].iloc[-1]), 
            xytext=(df["P2"].iloc[-1]-430,df["Depth"].iloc[-1]), fontsize=10,
            arrowprops=dict(facecolor='#333',width=.5,frac=.4,headwidth=4),color="#1e3d59")
